...
if(isset($_POST['add_product'])){
   ...
   $insert_query = mysqli_query($conn, "INSERT INTO `supplier`(name, price, quantity, shipping, description, image) VALUES('$p_name', '$p_price', '$p_quantity', '$p_shipping', '$p_description', '$p_image')") or die('query failed');

   if($insert_query){
      move_uploaded_file($p_image_tmp_name, $p_image_folder);
      $message[] = 'Supplied item added succesfully.';
   }else{
      $message[] = 'Supplied item could not be added.';
   }
};

if(isset($_GET['delete'])){
   $delete_id = $_GET['delete'];
   $delete_query = mysqli_query($conn, "DELETE FROM `supplier` WHERE id = $delete_id ") or die('query failed');
   if($delete_query){
      header('location:admin.php');
      $message[] = 'The following supplied item has been deleted';
   }else{
      header('location:admin.php');
      $message[] = 'The following supplied item could not be deleted';
   };
};
...
if(isset($_POST['update_product'])){
   ...
   $update_p_shipping = $_POST['update_p_shipping'];
   $update_p_description = $_POST['update_p_description'];
   $update_p_image = $_FILES['update_p_image']['name'];
   $update_p_image_tmp_name = $_FILES['update_p_image']['tmp_name'];
   $update_p_image_folder = 'uploaded_img/'.$update_p_image;

   $update_query = mysqli_query($conn, "UPDATE `supplier` SET name = '$update_p_name', price = '$update_p_price', quantity = '$update_p_quantity', shipping = '$update_p_shipping', description = '$update_p_description',image = '$update_p_image' WHERE id = '$update_p_id'");
...
<form action="" method="post" class="add-product-form" enctype="multipart/form-data">
   ...
   <input type="number" name="p_shipping" min="0" placeholder="Enter the shipping fee" class="box" required>
   ...
   <table>
      ...
      <tbody>
         ...
         <tr>
			...
			<td>RM<?php echo $row['shipping']; ?></td>
...
<section class="edit-form-container">
   ...
   if(isset($_GET['edit'])){
      $edit_id = $_GET['edit'];
      $edit_query = mysqli_query($conn, "SELECT * FROM `supplier` WHERE id = $edit_id");
   ...
   <form action="" method="post" enctype="multipart/form-data">
	  ...
	  <input type="number" min="0" class="box" required name="update_p_shipping" value="<?php echo $fetch_edit['shipping']; ?>">
