<?php
	...
	echo 'first name = ' .$fname . "<br> last name = " .$lname . "<br> email = " .$email . "<br> password = " .$password . "<br> confirmPassword = " .$confirmPassword . "<br> gender = " .$gender . "<br>";
        // count erros
        if (count($errors) === 0) {
            $insertQuery = "INSERT INTO adminInfo (firstName,lastName,email,password,gender,code,status)
            ...
            if ($insertInfo) {
                ...
                $sender = 'From: edwardjackson.my@gmail.com';
    ...
    if (isset($_POST['verify'])) {
        ...
        $otp_query = "SELECT * FROM adminInfo WHERE code = $otp";
        ...
        if (mysqli_num_rows($otp_result) > 0) {
            ...
            $update_query = "UPDATE adminInfo SET status = '$update_status' , code = $update_code WHERE code = $fetch_code";
    ...
    if (isset($_POST['login'])) {
        ...
        $emailQuery = "SELECT * FROM adminInfo WHERE email = '$email'";
        $email_check = mysqli_query($conn, $emailQuery);

        if (mysqli_num_rows($email_check) > 0) {
            $passwordQuery = "SELECT * FROM adminInfo WHERE email = '$email' AND password = '$password'";
    ...
    if (isset($_POST['forgot_password'])) {
        ...
        $emailCheckQuery = "SELECT * FROM adminInfo WHERE email = '$email'";
        ...
        if ($emailCheckResult) {
            ...
            if (mysqli_num_rows($emailCheckResult) > 0) {
                $code = rand(999999, 111111);
                $updateQuery = "UPDATE adminInfo SET code = $code WHERE email = '$email'";
...
if(isset($_POST['verifyEmail'])){
    ...
    $verifyQuery = "SELECT * FROM adminInfo WHERE code = $OTPverify";
    $runVerifyQuery = mysqli_query($conn, $verifyQuery);
    if($runVerifyQuery){
        if(mysqli_num_rows($runVerifyQuery) > 0){
            $newQuery = "UPDATE adminInfo SET code = 0";
...
if(isset($_POST['changePassword'])){
    ...
    if (strlen($_POST['password']) < 8) {
        ...
        if ($_POST['password'] != $_POST['confirmPassword']) {
            ...
            $updatePassword = "UPDATE adminInfo SET password = '$password' WHERE email = '$email'";
